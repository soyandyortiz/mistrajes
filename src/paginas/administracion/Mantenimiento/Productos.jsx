...
export default function Productos() {
  ...
  const addFoto = async (e) => {
    ...
    const disponibles = 5 - formData.fotos.length;
    if (disponibles <= 0) return toast.error('Ya alcanzaste el límite de 5 fotos por producto');
    const validas = [];
    for (const f of files.slice(0, disponibles)) {
      if (f.size > 5 * 1024 * 1024) { toast.error(`"${f.name}" supera el límite de 5 MB`); continue; }
      try {
        const webp = await convertirAWebP(f);
        validas.push({ file: webp, url: URL.createObjectURL(webp), name: webp.name, isNew: true });
      } catch {
        toast.error(`No se pudo procesar "${f.name}"`);
      }
    }
    if (validas.length) setFormData(prev => ({ ...prev, fotos: [...prev.fotos, ...validas] }));
...
<p className="font-bold text-[var(--text-primary)] text-sm">{p.nombre}</p>
       ...
       {(currentTab === 'nuevo' || currentTab === 'editar') && (
           <form onSubmit={guardarProducto} className="glass-card p-6 md:p-10 animate-in slide-in-from-right-4 space-y-10">
               ...
               {/* 3. Fotos */}
               <div>
                  <h3 className="text-sm font-black uppercase tracking-widest text-[var(--color-primary)] mb-6 border-b border-[var(--color-primary)]/20 pb-4">3. Material Visual (Máx 5)</h3>
                  ...
                  <p className="text-[10px] text-[var(--text-muted)] mt-3">Máximo <strong>5 fotos</strong> por producto/traje. Tamaño máximo por imagen: <strong>5 MB</strong>. Formatos: JPG, PNG, WEBP.</p>
               </div>
