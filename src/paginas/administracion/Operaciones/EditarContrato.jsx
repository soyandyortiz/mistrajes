...
import {
  ...
  Plus, Trash2, Loader2, Search, Lock, Unlock, CheckCircle2,
  ShieldCheck, Edit2, AlertTriangle,
} from 'lucide-react';
...
export default function EditarContrato({ contrato, onVolver, onGuardado }) {
  ...
  // Inicia en true igual que NuevoContrato: devolución se auto-calcula +24h desde salida
  ...
  const handleSalidaChange = (field, val) => {
    ...
    if (nf.fecha_salida_date) {
      ...
      if (!isNaN(salida)) {
        // Auto-calcular devolución (+24h) si está bloqueada
        ...
        // Auto-sugerir fecha de evento = misma fecha de salida si aún no tiene valor
  ...
  const confirmarCantidad = (prodId) => setProductosCarrito(prev => prev.map(p => {
    if (p.id !== prodId) return p;
    if (!p.cantidad_total || p.cantidad_total < 1) return (toast.error('Cantidad mínima: 1'), p);
    return { ...p, fase: p.piezas.length === 0 ? 'confirmado' : 'tallas' };
  }));
  ...
  const updateTalla = (prodId, piezaId, talla, valor) => {
    ...
    setProductosCarrito(prev => prev.map(p => p.id !== prodId ? p : {
      ...p, piezas: p.piezas.map(pz => pz.id !== piezaId ? pz : { ...pz, tallasCantidades: { ...pz.tallasCantidades, [talla]: v } })
    }));
  };
  ...
  const confirmarTallas = (prodId) => setProductosCarrito(prev => prev.map(p => {
    if (p.id !== prodId) return p;
    if (!piezasValidas(p)) return (toast.error('La suma de tallas debe ser igual a la cantidad total del producto.'), p);
    return { ...p, fase: 'confirmado' };
  }));

  const editarTallas = (prodId) => setProductosCarrito(prev => prev.map(p => p.id === prodId ? { ...p, fase: 'tallas' } : p));
  ...
  const nuevoSaldo = Math.max(0, total - Number(contrato.anticipo_pagado || 0));
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-64 shrink-0">
          <div className="glass-card p-6 sticky top-24">
            ...
            <button onClick={onVolver} className="w-full mt-6 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors">
          ...
          <div className="glass-card p-8 md:p-10 relative overflow-hidden min-h-[500px] flex flex-col">
            ...
            {step === 1 && (
              <div className="animate-in slide-in-from-right-4 duration-300 flex-1 flex flex-col">
                ...
                <p className="text-xs text-[var(--text-muted)] mb-8">Modifica las fechas del contrato. El campo de devolución puede bloquearse para auto-calcular +24h desde la salida.</p>
            ...
            {step === 2 && (
              <div className="animate-in slide-in-from-right-4 duration-300 flex-1 flex flex-col">
                ...
                <p className="text-xs text-[var(--text-muted)] mb-6">Modifica los productos del contrato. Al guardar, el saldo pendiente se recalculará manteniendo los abonos ya registrados.</p>
                  ...
                  <>
                    ...
                    {productosCarrito.length === 0 ? (
                      <div className="flex-1 flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="text-xs text-[var(--text-muted)] opacity-60 mt-1">Busca y agrega al menos un producto</p>
                      </div>
                    ) : (
                      <div className="space-y-4 flex-1">
                        {productosCarrito.map(prod => (
                          <div key={prod.id} className={`border rounded-2xl overflow-hidden transition-all ${prod.fase === 'confirmado' ? 'border-[var(--color-primary)]/30 bg-[var(--color-primary-dim)]/10' : 'border-[var(--border-soft)] bg-[var(--bg-surface-2)]'}`}>
                            {/* Cabecera producto */}
                            <div className="flex items-center justify-between px-5 py-4">
                              <div className="flex items-center gap-3">
                                {prod.fase === 'confirmado' && <CheckCircle2 className="h-4 w-4 text-[var(--color-primary)] shrink-0" />}
                                <div>
                            ...
                            {prod.fase === 'cantidad' && (
                              <div className="px-5 pb-4 space-y-3 border-t border-[var(--border-soft)]">
                                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] mt-3">Cantidad total</p>
                                <div className="flex gap-3 items-center">
                                  <input type="number" min="1" className="input-guambra w-28 text-center font-black text-lg"
                                  ...
                                  <button type="button" onClick={() => confirmarCantidad(prod.id)} className="btn-guambra-primary !py-2.5 !text-xs flex items-center gap-1.5">
                                    <ChevronRight className="h-4 w-4" /> Confirmar Cantidad
                                  </button>
                                </div>
                              </div>
                            )}
                            ...
                            {prod.fase === 'tallas' && (
                              <div className="px-5 pb-5 border-t border-[var(--border-soft)]">
                                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--color-primary)] mt-4 mb-3">Distribución de tallas por pieza (Total: {prod.cantidad_total})</p>
                                <div className="space-y-4">
                                  {prod.piezas.map(pz => {
                                    const asignado = totalAsignado(pz);
                                    const ok = asignado === prod.cantidad_total;
                                    return (
                                      <div key={pz.id} className={`p-4 rounded-xl border ${ok ? 'border-green-500/30 bg-green-500/5' : 'border-[var(--border-soft)] bg-[var(--bg-surface)]'}`}>
                                        <div className="flex items-center justify-between mb-3">
                                          <p className="text-xs font-black text-[var(--text-primary)] uppercase tracking-wider">{pz.nombre}</p>
                                          <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${ok ? 'bg-green-500/20 text-green-400' : 'bg-amber-500/20 text-amber-400'}`}>
                                            {asignado}/{prod.cantidad_total}
                                          </span>
                                        </div>
                                        <div className="flex flex-wrap gap-3">
                                          {pz.tallasDisponibles.length > 0 ? pz.tallasDisponibles.map(({ talla }) => (
                                            <div key={talla} className="flex flex-col items-center gap-1">
                                              <span className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest">{talla}</span>
                                              <input type="number" min="0" max={prod.cantidad_total}
                                                className="input-guambra w-16 text-center text-sm font-black !py-1.5"
                                                value={pz.tallasCantidades[talla] || 0}
                                                onChange={e => updateTalla(prod.id, pz.id, talla, e.target.value)}
                                              />
                                            </div>
                                          )) : (
                                ...
                                <button type="button" onClick={() => confirmarTallas(prod.id)}
                                  ...
                                  className="btn-guambra-primary mt-4 !py-2.5 !text-xs flex items-center gap-2 disabled:opacity-40">
                                  <CheckCircle2 className="h-4 w-4" /> Confirmar Tallas
                                </button>
                              </div>
                            )}
                          </div>
            ...
            {step === 3 && (
              <div className="animate-in slide-in-from-right-4 duration-300 flex-1 flex flex-col">
                ...
                <p className="text-xs text-[var(--text-muted)] mb-8">Los abonos ya registrados se mantienen intactos. El saldo pendiente se recalcula automáticamente.</p>
                  ...
                  <div className="glass-card p-6 space-y-4 border border-[var(--border-soft)]">
                    ...
                    <div className="pt-3 border-t border-[var(--border-soft)] grid grid-cols-3 gap-3 text-center">
                      <div className="bg-green-500/10 rounded-xl p-3 border border-green-500/20">
                        ...
                        <p className="font-black text-green-400">${Number(contrato.anticipo_pagado || 0).toFixed(2)}</p>
                      </div>
                      <div className={`rounded-xl p-3 border ${nuevoSaldo > 0 ? 'bg-red-500/10 border-red-500/20' : 'bg-green-500/10 border-green-500/20'}`}>
                        <p className={`text-[9px] font-black uppercase tracking-widest mb-1 ${nuevoSaldo > 0 ? 'text-red-400' : 'text-green-400'}`}>Nuevo saldo</p>
                        <p className={`font-black ${nuevoSaldo > 0 ? 'text-red-400' : 'text-green-400'}`}>${nuevoSaldo.toFixed(2)}</p>
                      </div>
                    ...
                    {nuevoSaldo < Number(contrato.saldo_pendiente || 0) && (
                ...
                <div className="flex justify-between mt-8 pt-6 border-t border-[var(--border-soft)]">
                  ...
                  <button onClick={handleGuardar} disabled={guardando}
                    ...
                    {guardando ? <><Loader2 className="h-4 w-4 animate-spin" /> Guardando...</> : <><CheckCircle2 className="h-4 w-4" /> Guardar Cambios</>}
                  </button>
                </div>
              </div>
            )}
